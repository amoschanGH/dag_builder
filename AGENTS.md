...
- Buffer insertion is explicit and structural only. Preserve first-delivery-wins behavior and reject conflicting IDs without overwriting state; allow multiple blocks at the same `(source, round)` slot to model equivocation, while quorum counts distinct sources.
